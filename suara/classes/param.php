...
class param {
	...
	public function __construct() {
		...
		$this->route_config = s_core::load_config('domain', SITE_URL) ? s_core::load_config('domain', SITE_URL) : s_core::load_config('domain', 'default');
	...
	public function route_m() {
		...
		$m = $this->safe_deal($m);
		if (empty($m)) {
			return $this->route_config['module'];
		} else {
	...
	public function route_a() {
		...
		$a = $this->safe_deal($a);
		if (empty($a)) {
			return $this->route_config['action'];
		} else {
	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		$var = s_core::load_config('system','cookie_pre').$var;
		$_COOKIE[$var] = $value;
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE'), $time, s_core::load_config('system','cookie_path'), s_core::load_config('system','cookie_domain'), $s);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE'), $time, s_core::load_config('system','cookie_path'), s_core::load_config('system','cookie_domain'), $s);
		}
	}
